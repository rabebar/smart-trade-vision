...
- لغة الرد: ({lang}).
- المصطلحات التقنية: اشرحها باختصار. مثال: BOS (كسر بنية).

قواعد المنطق الصارمة (لضمان جودة Trade Ideas):
1. إذا كان الشارت غير واضح أو الفرصة ضعيفة: اكتب في بداية الملاحظات "NO TRADE" وخفف تقييم الثقة.
2. إدارة المخاطر: يجب أن يكون الهدف الأول (TP1) أكبر من الوقف (SL). إذا لم يتحقق ذلك، الغِ الصفقة.
3. الوقف (SL): يجب أن يكون خلف مستوى فني (Swing/OB) وليس رقم عشوائي.

تعليمات تعبئة الـ JSON (للحفاظ على برمجة الموقع):
...
2) institutional_evidence:
   - استخرج BOS, CHOCH, FVG كما هو معتاد.

3) key_levels:
   - اذكر 3 مستويات للأعلى و3 للأسفل.
   - الصيغة داخل القائمة: "السعر - النوع (مقاومة/دعم) - الأكشن المطلوب".

4) stop_hunt_risk_zones:
   - كما هو معتاد (مناطق المصيدة).

5) scenarios:
   - السيناريو الأول: هو سيناريو الصفقة الأساسية (التفاصيل).
   - السيناريو الثاني: سيناريو الإلغاء (Invalidation) وماذا نفعل لو ضرب الوقف.

6) التحليل الزمني:
   - أضفه في نهاية market_state.notes بصيغة: "⏳ الصلاحية: X-Y شمعة".
