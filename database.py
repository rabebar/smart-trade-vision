...
def migrate_database():
    """
    وظيفة الإصلاح التلقائي: تقوم بفتح الأقفال وتنظيف البيانات 
    بشكل آمن وذكي لتجنب أخطاء التشغيل.
    """
    try:
        with engine.begin() as conn:
            # محاولة تفعيل الحسابات، إذا لم يجد العمود سيتخطى الأمر ولن ينهار السيرفر
            conn.execute(text("UPDATE users SET is_verified = TRUE WHERE is_verified = FALSE"))
            
            # توحيد الإيميلات لأحرف صغيرة لمنع تعارض البيانات مستقبلاً
            if not SQLALCHEMY_DATABASE_URL.startswith("sqlite"):
                conn.execute(text("UPDATE users SET email = LOWER(TRIM(email))"))
                
            print("✅ تم فحص الخزنة السحابية وتأمين كافة الحسابات بنجاح")
    except Exception as e:
        # في حال وجود نقص في الأعمدة، السيرفر سيكمل التشغيل وستقوم create_all بالمعالجة
        print("⚠️ ملاحظة: السيرفر يعمل وسيقوم بتحديث هيكل البيانات آلياً.")

# بناء الجداول الأساسية (فقط إذا لم تكن موجودة)
